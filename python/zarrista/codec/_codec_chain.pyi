...
class CodecChain:
    ...
    A codec chain holds the array-to-array codecs ("filters"), then one
    array-to-bytes codec ("serializer"), then the bytes-to-bytes codecs
    ("compressors"). The chain applies them in that order to encode, and in the
    reverse order to decode.
